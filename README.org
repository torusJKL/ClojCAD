...
#+HTML: <p align="center">
#+HTML: <img src="https://www.clojcad.com/public/clojcad-viewer.png" width="800" />
#+HTML: </p>
...
Toggle model or tag visibility. Accepts either a model name (existing
...
Dynamically add or remove tagged sub-shapes on displayed models at
...
Added tags are ephemeral — they are lost when the model is re-evaluated
...
Since =all-ns= and =ns-publics= are not available in ClojureScript's
...
- =ClojCAD.scene.manager= :: Manages the scene: watches the =params=
     atom, drives tessellation and viewer updates. Provides overloaded
     =show-model=/=hide-model= with filter-map dispatch, =toggle-model=,
     =show-all=/=hide-all=.
- =ClojCAD.scene.api= :: Query API for the scene: =list-objects=
     (with optional =:tag=, =:visibility=, =:name-matching= filters)
     and =list-tags=. Forwards visibility functions from manager.
